...
export function ManagerDashboard() {
  ...
  const fetchAnalytics = useCallback(async () => {
    try {
      ...
      const [analyticsRes, topItemsRes, hourlyRes] = await Promise.all([
        fetch(
          `/api/analytics?startDate=${start.toISOString()}&endDate=${end.toISOString()}&compareWithPrevious=${compareWithPrevious}`,
        ),
        fetch(
          `/api/analytics/top-items?startDate=${start.toISOString()}&endDate=${end.toISOString()}&limit=10`,
        ),
        fetch(
          `/api/analytics/hourly?startDate=${start.toISOString()}&endDate=${end.toISOString()}`,
        ),
      ]);
  ...
  const avgOrderValueChange = analytics?.comparison
    ? calculatePercentageChange(
        ...
        analytics.comparison.avgOrderValue,
      )
    : undefined;
      ...
      <header className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Monitor your dining room in real time. Metrics auto-refresh every 30
            seconds.
          </p>
        </div>
          ...
          <>
            ...
            {analytics.hourlyData.length > 0 ? (
              ...
              <div className="flex h-64 items-center justify-center rounded-2xl border border-dashed border-border bg-background/50">
                <p className="text-sm text-muted-foreground">
                  No hourly data available. Orders will appear here once they
                  start coming in.
                </p>
              </div>
            )}
