...
const STATUS_FLOW: StaffOrderStatus[] = [
  "pending",
  "preparing",
  "ready",
  "completed",
  ...
  const fetchOrders = useCallback(async () => {
    try {
      const response = await fetch(
        "/api/orders?status=pending,preparing,ready,completed&limit=100",
      );
      if (!response.ok) {
    ...
    <section className="flex flex-col gap-6">
      <header className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Tap a card to progress an order through the kitchen workflow. Orders
                                ...
                                {prevStatus ? (
                                  <button
                                    ...
                                    onClick={() =>
                                      moveOrder(
                                        order.id,
                                        prevStatus as StaffOrderStatus,
                                      )
                                    }
                                  >
                                    ←{" "}
                              ...
                              {nextStatus ? (
                                <button
                                  ...
                                  onClick={() =>
                                    moveOrder(
                                      order.id,
                                      nextStatus as StaffOrderStatus,
                                    )
                                  }
                                >
                                  Move to{" "}
                                  {columnLabel(nextStatus as StaffOrderStatus)}
                                </button>
                              ) : (
                                <span className="text-[11px] uppercase tracking-wide text-muted-foreground">
                                  Complete
        ...
        <>
          ...
          <div className="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border border-border bg-card p-6 shadow-2xl">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              You are about to cancel order #{cancelConfirmation.orderNumber}.
              <br />
