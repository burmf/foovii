-- Supabase schema helpers for Foovii orders, history, and analytics
-- Apply after provisioning the menu schema (menu-schema.sql).
...
-- Ensure order numbers are unique per store
ALTER TABLE public.orders
  ADD CONSTRAINT IF NOT EXISTS orders_store_slug_order_number_unique
  UNIQUE (store_slug, order_number);

-- Useful indexes for history queries
...
-- Enable RLS; restrict writes to service role, allow read for authenticated clients
...
-- Flattened history view (helps with Supabase table editor & API filters)
...
-- Aggregated metrics view backing the manager dashboard
